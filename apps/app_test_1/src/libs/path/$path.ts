...
export const pagesPath = {
  ...
  "local_storage": {
    $url: (url?: { hash?: string | undefined } | undefined) => ({ pathname: '/local-storage' as const, hash: url?.hash })
  },
  "posts": {
  ...
  "router": {
    _id: (id: string | number) => ({
      $url: (url?: { query?: OptionalQuery_bfn3mb | undefined, hash?: string | undefined } | undefined) => ({ pathname: '/router/[id]' as const, query: { id, ...url?.query }, hash: url?.hash })
    }),
